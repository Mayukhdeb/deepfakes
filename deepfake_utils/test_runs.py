...
dummy_conv = autoencoder._ConvLayer(input_features = 3, output_features  =  10)
pred = dummy_conv(image)
print ("batch through dummy conv size: ", pred.size())


dummy_upscale = autoencoder._UpScale(input_features = 10, output_features  =  4) 
pred  = dummy_upscale(pred)
print ("batch through dummy upsample size: ", pred.size())

dummy_shuffle = autoencoder._PixelShuffler()
